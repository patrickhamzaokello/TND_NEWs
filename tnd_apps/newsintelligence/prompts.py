...
{{
  ...
  "long_summary": "<THE CHRONOLOGICAL ACCOUNT — 2-4 paragraphs combining all reporting in order: what happened first, what developed next, where things stand now, what remains unresolved. Include specific names, figures, dates, and places. Attribute single-source claims. This is the definitive 'what happened' narrative — put ALL the chronological facts here.>",

  "overview": "<BACKGROUND AND CONTEXT ONLY — 1-3 short paragraphs. Do NOT restate what happened chronologically; that belongs in long_summary and would just duplicate it. Instead explain: what led up to this (prior context, not already-covered chronology), the broader pattern or precedent this fits into, and what makes it notable beyond the immediate facts. Use the related earlier stories listed above for this background where relevant. If there genuinely is no context beyond the reporting itself, write ONE short paragraph rather than padding — never repeat long_summary's content to fill space.>",
